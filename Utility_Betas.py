...
1. Create dictionary of companies to include in the sample
    FYI: found  that looking up company info is more efficient using si.get_quote_data(ticker)
    than yf.Ticker(ticker), so using the former approach
'''

company_dict = {}

#Also save market caps, price to book values for later 
marketCap_dict = {}
priceToBook_dict = {}
...
for ticker, name in company_dict_input.items():
    try:
        ...
        if name.strip() == longName.strip():
            ...
            marketCap_dict.update({ticker:marketCap})
            priceToBook_dict.update({ticker:priceToBook})
            print('Including '+ticker+': '+longName+' in the analysis')
        else: 
...
'''
VI. Calculate Annual Returns for Utility Stocks 
'''

'''
A. Get yearly price data
'''

'''
1. Pull yearly prices
'''

yearly_prices0 = prices_table.loc[pd.to_datetime(prices_table['Date']).dt.dayofyear == 1]

'''
2. For QA purposes, keep only those companies for which also calculated a Beta
'''

yearly_prices = yearly_prices0[ptf_columns]

'''
B. Output to new worksheet
'''
...
wb.sheets.add(name='Annual Returns', after='Chart')
ws_ar = wb.sheets['Annual Returns']

'''
2. Output the yearly prices table to the worksheet
'''

ws_ar.range('A1').value = yearly_prices
ws_ar.range('A:A').api.Delete()

'''
C. For each company, calculate Annual Return
'''

last_column_num = ws_ar.range(1,1).end('right').column
last_column_name = ws_ar.range(1,last_column_num).value
last_row = ws_ar.range(1,1).end('down').row

while last_column_name != 'Imputed_Price':
    #Series address row & column keys
    series_address = ws_ar.range(1,last_column_num).get_address(0,0)
    series_col = ''.join(LettersRegex.findall(series_address))
    series_colnum = ws_ar.range(series_address).column
    
    #Create Column Headers for the series
    ticker = ws_ar.range(1,series_colnum).value
    ws_ar.range(1,series_colnum+1).value = ticker + '_Annual_Return'

    return_col = ''.join(LettersRegex.findall(ws_ar.range(1,series_colnum+1).get_address(0,0)))

    if ws_ar.range(2,series_colnum).value is None:
        firstobs_address = ws_ar.range(series_address).end('down')
        firstobs_row = ws_ar.range(firstobs_address).row
        
    else:
        firstobs_address = ws_ar.range(2,series_colnum)
        firstobs_row = 2
        
    #Calculate Annual Return 
    return_formula = "=({col}{r1}-{col}{r0})/{col}{r0}".format(col=series_col,r1=firstobs_row+1,r0=firstobs_row)
    return_range = "{return_col}{rstart}:{return_col}{last_row}".format(return_col=return_col,rstart=firstobs_row+1,last_row=last_row) 
    ws_ar.range(firstobs_row+1,series_colnum+1).value = return_formula

    ws_ar.range(firstobs_row+1,series_colnum+1).api.AutoFill(ws_ar.range(return_range).api,0)
    
    #If adjacent series is not Imputed_Price, insert a blank column
    if ws_ar.range(1,series_colnum-1).value != 'Imputed_Price':
        ws_ar.range("{col}:{col}".format(col=series_col)).insert(shift='right') 
        ...
        last_column_name = 'Imputed_Price'



'''
VII. Post COVID-19 Beta Analysis
'''

'''
A. Pull the relevant post COVID-19 data
'''

'''
1. Pull from excel into a data frame
'''

last_row = ws_bc.range(1,1).end('down').row

rtext = "A1:K{last_row}".format(last_row=last_row)

avgdat = ws_bc.range(rtext).options(pd.DataFrame, index=0, header=1).value
...
avgdat_c19 = avgdat[['Date','Average_Beta','NYA_Deviation','Average_Deviation']]

avgdat_c19 = avgdat_c19.rename(columns={'Average_Beta':'Average_Utility_Beta','Average_Deviation':'Average_Utility_Deviation'})

'''
3. Keep only the relevant rows
'''

avgdat_c19 = avgdat_c19[avgdat_c19['Date']>'2018-01-01'] 

'''
B. Output the data in excel. Will create chart manually
'''
...
wb.sheets.add('Betas & COVID-19', before='Prices')
ws_bc19 = wb.sheets['Betas & COVID-19']

'''
2. Output the data
'''

ws_bc19.range('A1').value = avgdat_c19
ws_bc19.range('A:A').api.Delete()

'''
3. Calculate Product of Deviation
'''

ws_bc19.range('E1').value = 'Product_of_Deviation'
ws_bc19.range('E2').value = '=C2*D2'

last_row = ws_bc19.range(1,1).end('down').row

pd_range = "E2:E{last_row}".format(last_row=last_row)

ws_bc19.range("E2").api.AutoFill(ws_bc19.range(pd_range).api,0)

ws_bc19.range("F1").value = 'To avoid coding complication: create chart manually'
ws_bc19.range("F2").value = 'Average_Utility_Beta on Primary Axis'
ws_bc19.range("F3").value = 'Product_of_Deviation on Secondary Axis'

'''
VIII. Create summary stats worksheet
'''
...
#Add new worksheet
wb.sheets.add('Summary Stats', before='Chart')
ws_ss = wb.sheets['Summary Stats']

#Add column headers
ws_ss.range('A1').value = 'Company'
ws_ss.range('B1').value = 'Ticker'
ws_ss.range('C1').value = 'MarketCap as of ' + str(date.today())
ws_ss.range('D1').value = 'PricetoBook as of ' + str(date.today())
ws_ss.range('E1').value = 'Price Series Start Date'
ws_ss.range('F1').value = 'Beta Series Start Date'
ws_ss.range('G1').value = 'Average Beta'
ws_ss.range('H1').value = 'Average Adjusted Beta'
ws_ss.range('I1').value = 'Average Annual Return'
...
last_column_num_bc = ws_bc.range(1,1).end('right').column
last_column_num_ar = ws_ar.range(1,1).end('right').column
...
for ticker in company_dict2:
    ...
    #Fill in Company Name, Ticker, MarketCap, and Price to Book
    ws_ss.range(row,1).value = company_dict[ticker]
    ws_ss.range(row,2).value = ticker
    ws_ss.range(row,3).value = marketCap_dict.get(ticker)
    ws_ss.range(row,4).value = priceToBook_dict.get(ticker)
    
    #If have 260 return obs, get info from Beta Calcs & Annual Returns worksheet
    if company_dict2[ticker]==1: 
        for i in range(last_column_num_bc):
            if ticker == ws_bc.range(1,i+1).value:

                #Need to assign as beta_col2, adj_beta_col2 to prevent macro declaration issues
        ...
        ws_ss.range(row,5).value = "='Beta Calcs'!C{prices_r1}".format(prices_r1=prices_r1)
    
        #Beta Series Start Formula
        ws_ss.range(row,6).value = "='Beta Calcs'!C{beta_r1}".format(beta_r1=beta_r1)
    
        #Average Beta/Adj Beta Formula
        ws_ss.range(row,7).value = "=AVERAGE('Beta Calcs'!{beta_col}:{beta_col})".format(beta_col=beta_col2)
        ws_ss.range(row,8).value = "=AVERAGE('Beta Calcs'!{adj_beta_col}:{adj_beta_col})".format(adj_beta_col=adj_beta_col2)
        
        for i in range(last_column_num_ar):
            if ticker == ws_ar.range(1,i+1).value:
                
                global ar_col
                
                series_colnum = ws_ar.range(1,i+1).column
                ar_col = ''.join(LettersRegex.findall(ws_ar.range(1,series_colnum+1).get_address(0,0)))
                
                break
                
        #Average Annual Return Formula
        ws_ss.range(row,9).value = "=AVERAGE('Annual Returns'!{ar_col}:{ar_col})".format(ar_col=ar_col)
        
    #Otherwise indicate if dont have enough return obs
    else:
        ws_ss.range(row,5).value = "NA - less than 260 weekly return observations"
    
    row = row + 1 

'''
IX. Format the workbook
'''
